...
**91% of scheduled candidates show up. Hiring nurses? Book the interviews.**
...
**Components (no screenshots):** healthcare candidates widget (#6 in snippets file) after checklist; evergreen calendar card (#7) before FAQ. No city/date swap needed.
...
**Contents sidebar + About panel (added Jul 21 — match the live post template):** left "Contents" nav links to these section IDs (add the id to each matching H2): `#checklist` The 7-point checklist for evaluating a nurse job fair · `#types` Types of nurse job fairs, honestly compared · `#hospital-systems` What do large hospital systems actually do? · `#jobfairx` Where JobFairX fits (and where it doesn't) · `#faq` FAQ. Right "About this article" panel: Published = publish date, Updated = same at launch; bump Updated at every content refresh.
