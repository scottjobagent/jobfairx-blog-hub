...
**Contents sidebar + About panel (added Jul 21 — match the live post template):** left "Contents" nav links to these section IDs (add the id to each matching H2): `#what-is-it` What is cost per hire? · `#formula` How do you calculate cost per hire? · `#benchmarks` Cost per hire benchmarks for 2026 · `#why-rising` Why is cost per hire going up? · `#lower-it` 6 ways to lower cost per hire · `#events` Where do hiring events fit? · `#faq` FAQ. Right "About this article" panel: Published = publish date, Updated = same at launch; bump Updated at every content refresh.
